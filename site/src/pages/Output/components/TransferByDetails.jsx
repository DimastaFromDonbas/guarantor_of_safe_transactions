                        ...
                        <div className="alert-block">
                            {transfers ? <>
                                <div className="output-description-info-block">
                                    <div style={ SSizeBlockV1 } className="output-id dilit-block">ID</div>
                                    <div style={ SSizeBlockV2 } className="output-date dilit-block">Платежная система</div>
                                    <div style={ SSizeBlockV3 } className="output-date">Номер банковской карты </div>
                                    <div style={ SSizeBlockV4} className="output-sum">Сумма</div>
                                    <div style={ SSizeBlockV2 } className="output-sum">Время перевода</div>
                                    <div style={ SSizeBlockV2 } className="output-sum">Состояние перевода </div>
                                </div>
                                {transfers?.sort((a, b) => a.id - b.id)?.map((item, index) => <div style={{ justifyContent: "space-around" }} className="flex-info-block" key={index}>
                                    <div style={ SSizeBlockV1 } className="output-id dilit-block">{item.id}</div>
                                    <div style={ SSizeBlockV2 } className="output-date dilit-block">{item.paymantSystem}</div>
                                    <div style={ SSizeBlockV3 } className="output-date">{item.walletNumber}</div>
                                    <div style={ SSizeBlockV4 } className="output-sum">{item.score}</div>
                                    <div style={ SSizeBlockV2 } className="output-sum">{item.time}</div>
                                    <div style={ SSizeBlockV2 } className="output-sum">{transferStatusMock[item?.status === 0 ? item?.status : item?.status - 1]}</div>
                                </div>)}
                            </>
                                :
