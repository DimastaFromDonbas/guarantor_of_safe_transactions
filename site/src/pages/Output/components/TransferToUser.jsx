                ...
                <div className="alert-block">
                    {transfersToUser ? <>
                        <div className="output-description-info-block">
                            <div style={ SSizeBlock } className="output-id dilit-block">ID</div>
                            <div style={ SSizeBlock } className="output-date">Email пользователя</div>
                            <div style={ SSizeBlock } className="output-sum">Сумма</div>
                            <div style={ SSizeBlock } className="output-sum">Время перевода</div>
                            <div style={ SSizeBlock } className="output-sum">Состояние перевода </div>
                        </div>
                    {transfersToUser?.sort((a, b) => a.id - b.id)?.map((item, index) => <div style={{ justifyContent: "space-around" }} className="flex-info-block" key={index}>
                        <div style={ SSizeBlock } className="output-id dilit-block">{item.id}</div>
                        <div style={ SSizeBlockUnickl } className="output-date">{item.receiverEmail}</div>
                        <div style={ SSizeBlock } className="output-sum">{item.score}</div>
                        <div style={ SSizeBlock } className="output-sum">{item.time}</div>
                        <div style={ SSizeBlock } className="output-sum">{statuses[item?.status === 0 ? item?.status : item?.status - 1]}</div>
                    </div>)}
                            </>
                                :
